...
pub fn reparse_subset(
    ...
    if simple_ranges.is_empty() {
            ...
            })
        {
            ...
            for p in prefix_parse.iter().skip(form_error_start) {
                result.errors.insert(
                    ...
                    CompileErr(p.loc(), "bad form".to_string()),
                );
            }
        }
        ...
        if let Ok(suffix_parse) = parse_sexp(
            ...
            if !suffix_parse.is_empty() {
                if let Ok(body) = compile_bodyform(suffix_parse[suffix_parse.len() - 1].clone()) {
                    took_exp = true;
    ...
    for (i, r) in simple_ranges.iter().enumerate() {
        ...
        hash.append(
            &mut format!("{} {} {}", i, simple_ranges.len(), parse_as_body)
                .as_bytes()
                .to_vec(),
        );
                ...
                match compile_helperform(opts.clone(), parsed[0].clone()) {
                    Ok(h) => {
                        if let Some(helper) = h.as_ref() {
                            ...
                            result.errors.insert(
                                hash.clone(),
                                CompileErr(parsed[0].loc(), "impossible helper form".to_string()),
                            );
                        }
                    }
                    Err(e) => {
...
pub fn combine_new_with_old_parse(
    ...
    let compile_with_dead_helpers_removed = new_compile.remove_helpers(&to_remove_helpers);
