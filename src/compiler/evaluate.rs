...
use crate::compiler::comptypes::{
    Binding, BindingPattern, BodyForm, CompileErr, CompileForm, CompilerOpts, HelperForm, LambdaData, LetData,
    LetFormKind,
};
...
pub struct LambdaApply {
    ...
    body: Rc<BodyForm>,
    env: Rc<BodyForm>,
    env_at_callsite: HashMap<Vec<u8>, Rc<BodyForm>>,
}
...
impl<'info> Evaluator {
    ...
    fn do_lambda_apply(
        ...
        let degraded_codegen = lambda_codegen(&lapply.lambda)?;
        Ok(Rc::new(degraded_codegen))
    }
                        ...
                        recognize_consed_env(arguments_to_convert[0].clone())
                    {
                        if is_first {
                            let evaluated_first = self.shrink_bodyform_visited(
                                allocator, &mut visited, prog_args, env, first, expand,
                            )?;
                            return Ok(evaluated_first);
                        } else {
                            let evaluated_rest = self.shrink_bodyform_visited(
                                allocator, &mut visited, prog_args, env, rest, expand,
                            )?;
