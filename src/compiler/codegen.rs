...
use crate::compiler::comptypes::{
    fold_m, join_vecs_to_string, list_to_cons, BodyForm, Callable, CompileErr,
    CompileForm, CompiledCode, CompilerOpts, DefunCall, HelperForm, InlineFunction,
    LetFormKind, PrimaryCodegen,
};
use crate::compiler::debug::{build_swap_table_mut, relabel};
use crate::compiler::frontend::compile_bodyform;
use crate::compiler::inline::{replace_in_inline, synthesize_args};
use crate::compiler::letform::{hoist_body_let_binding, process_helper_let_bindings};
...
fn start_codegen(
    ...
    let hoisted_bindings = hoist_body_let_binding(None, comp.args.clone(), comp.exp);
    let mut new_helpers = hoisted_bindings.0;
    let expr = hoisted_bindings.1;
    new_helpers.append(&mut comp.helpers.clone());
    let let_helpers_with_expr = process_helper_let_bindings(&new_helpers);
