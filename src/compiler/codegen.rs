...
fn hoist_body_let_binding(
    ...
    match body.borrow() {
        ...
        // Lambda can have synthetic helpers generated by their let bindings.
        // Extract them here and add them to the outer program's synthetic
        // helpers since they share the left env.
        BodyForm::Mod(l, true, program) => {
            let (new_helpers, new_expr) = hoist_body_let_binding(
                opts.clone(),
                outer_context.clone(),
                program.args.clone(),
                program.exp.clone()
            ...
            let updated_program = CompileForm { exp: new_expr, .. program.clone() };
            (new_helpers, Rc::new(BodyForm::Mod(
                l.clone(), true, updated_program
            )))
        }
...
fn start_codegen(
    ...
    let (mut new_helpers, expr) = hoist_body_let_binding(opts.clone(), None, comp.args.clone(), comp.exp);
