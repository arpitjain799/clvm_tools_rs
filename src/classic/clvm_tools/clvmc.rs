...
fn compile_clvm_text(
    ...
    if let Some(dialect) = detect_modern(allocator, assembled_sexp) {
        ...
        let run_program = run_program_for_search_paths(input_path, search_paths, false, None);
        let run_program_output =
            run_program.run_program(allocator, compile_invoke_code, input_sexp, None, None)?;
